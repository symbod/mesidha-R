...
```{r loading, include=TRUE, warning=FALSE, message=FALSE}
  ...
  devtools::install_github("symbod/MaxQuantHandler-R", build_vignettes = TRUE, dependencies = TRUE)
...
#### 1.1 Load Data
```{r}
data = data.table::fread("../../../Li2021_proteins_NaOmit_SL_irs.txt")[0:50,]
```
...
filtered_results <- mqhandleR::filter_protein_ids(data = data, protein_column = protein_column, organism = organism,
...
#### 1.4 Inspect Logging
```{r}
DT::datatable(filtered_results$Overview_Log)
```

```{r}
DT::datatable(filtered_results$Detailed_Log)
```
